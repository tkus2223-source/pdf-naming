...
def load_journal_map(path: Path) -> Dict[str, str]:
    ...
    try:
        ...
        return {str(k).strip().lower(): sanitize_text(str(v)) for k, v in data.items()}
    except Exception:
        return {}
...
def parse_from_filename(path: Path, journal_map: Dict[str, str]) -> PdfInfo:
    ...
    set_if_empty(info.title, stem, "filename")  # hard requirement fallback
    ...
    if not journal_candidate:
        lowered = stem.lower()
        for full_name, abbr in journal_map.items():
    ...
    if info.journal.value:
        tokens = [t for t in tokens if t != info.journal.value]
...
def detect_title_from_text(text: str) -> Optional[str]:
    for line in (sanitize_text(l) for l in text.splitlines()):
        if len(line) < 20:
            continue
...
def crossref_lookup(doi: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "pdf-renamer/2.0"})
    try:
...
def choose_journal_abbrev(journal: Optional[str], journal_map: Dict[str, str]) -> Optional[str]:
    if not journal:
        return None
    mapped = journal_map.get(journal.lower())
    if mapped:
        return mapped
    return abbreviate_journal(journal)
...
def plan_renames(folder: Path, journal_map: Dict[str, str], use_crossref: bool) -> List[RenamePlan]:
    ...
    for src in sorted(folder.glob("*.pdf")):
        ...
        # never blank title; must at least use filename stem
...
def write_report(path: Path, plans: List[RenamePlan], dry_run: bool) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "old_name", "new_name", "year", "year_source", "journal", "journal_source",
            "title", "title_source", "doi", "doi_source", "mode"
        ])
        for p in plans:
            w.writerow([
                p.src.name, p.dest.name,
                p.info.year.value or "", source_of(p.info.year),
                p.info.journal.value or "", source_of(p.info.journal),
                p.info.title.value or "", source_of(p.info.title),
                p.info.doi.value or "", source_of(p.info.doi),
...
def execute(plans: List[RenamePlan], dry_run: bool, log_dir: Path) -> Optional[Path]:
    ...
    for p in plans:
        ...
        logs.append((str(p.src), str(p.dest), status))

    if dry_run:
        return None

    log_dir.mkdir(parents=True, exist_ok=True)
    log_path = log_dir / "rename_log.csv"
    with log_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["old_path", "new_path", "status"])
        w.writerows(logs)
    return log_path
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Rename PDFs to '<year> <journal> <title>.pdf'")
    p.add_argument("--folder", default="./pdfs")
    p.add_argument("--journals", default="./journals.json")
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("--undo")
    p.add_argument("--log-dir", default="./logs")
    p.add_argument("--use-crossref", action="store_true", help="Enable DOI Crossref enrichment")
    p.add_argument("--report", default="report.csv", help="Write rename analysis report CSV")
    return p
...
def main() -> int:
    ...
    if args.undo:
        undo(Path(args.undo), args.dry_run)
        return 0
    ...
    plans = plan_renames(folder, journal_map, args.use_crossref)
